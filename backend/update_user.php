...
try {
    if ($conn->query($sql) === TRUE) {
        ...
        // ✅ Jika user adalah orang_tua, hubungkan anak yang memiliki NISN ini (jika ada) ke user ID ini
        if ($role === 'orang_tua' && isset($newNisn) && !empty($newNisn)) {
            $escaped_nisn = $conn->real_escape_string($newNisn);
            // Hubungkan anak yang memiliki NISN ini ke orang tua ini (tanpa menimpa NISN anak lain yang sudah terhubung)
